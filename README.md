...
- [DataManager.py](data_manager.py): This script is responsible for the extracting and strcturing the data.
- [ModelsTrainer.py](models_trainer.py):This script is responsible for training the Gaussian Mixture Models (GMM) for each speaker.
...
- [FeaturesExtractor.py](features_extractor.py):This script is responsible for extracting the MFCC features from the .wav files.
- [SilenceEliminator.py](silence_eliminator.py): Silence eliminator from .wav files to speed/clean and optimize the input files for the features extraction.
